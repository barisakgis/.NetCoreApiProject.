...
namespace WebApi.Utilities.Formatters
{
    public class CsvOutputFormatter : TextOutputFormatter
    {
        ...
        // hangi istekler csv formatında cevap alabislin diye 
        ...
        public override async Task WriteResponseBodyAsync(OutputFormatterWriteContext context, Encoding selectedEncoding)
        {
           var response = context.HttpContext.Response;
            ...
            if(context.Object is IEnumerable<BookDto>)  // liste dönerse 
            {
                foreach(var book in (IEnumerable<BookDto>)context.Object)   // objeden çıkartma yapıyoruz 
                {
                    FormatCsv(buffer , book);
                }

            }
            else  // tek bişey dönerse 
            {
